...
fn main() {
    ...
    let input2 = input.to_vec();

    let oxygen_word = get_rating(input, 0, &|digit_count: DigitCount| -> char {
    ...
    let co2_word = get_rating(input2, 0, &|digit_count: DigitCount| -> char {
        if digit_count.0 <= digit_count.1 {
            '0'
        } else {
            '1'
...
fn get_rating<'a, I>(
    ...
    let count = get_digit_count(items, position);
    let discriminant = mapper(count);
    let new_input = items2.iter()
    ...
    return get_rating(new_input, position + 1, mapper);
}

fn get_digit_count<'a, I>(input: I, position: usize) -> DigitCount
    where
        I: IntoIterator<Item = &'a str> {
    input
        .into_iter()
